...
# 2018 Annual Average (A18)
A18 <- df %>%
  filter(collection_date > ymd('2018-01-01') & 
           collection_date < ymd('2019-01-01')) %>%
  group_by(site_code) %>%
  dplyr::summarize(length_A18_mu=mean(length_mu, na.rm=T),
                   length_A18_sd=sd(length_mu, na.rm=T),
                   d100_A18_mu=mean(d100_mu, na.rm=T),
                   d100_A18_sd=sd(d100_mu, na.rm=T),
                   biomass_A18_mu=mean(biomass_mu, na.rm=T),
                   biomass_A18_sd=sd(biomass_mu, na.rm=T),
                   shannon_A18_mu=mean(shannon, na.rm=T),
                   shannon_A18_sd=sd(shannon, na.rm=T))

# Baseline = 2018_annual_mean, 2018_annual_standard deviation,
df <- A18 %>%
  right_join(df) %>% 
  select(site_code, collection_date, qtr, everything()) %>%
  arrange(site_code, collection_date) %>%
  filter(collection_date < ymd('2018-05-01'))
...
df <- df %>%
  mutate(RR_length = (length_mu-length_A18_mu) / length_A18_mu ,
         RR_d100 = (d100_mu-d100_A18_mu) / d100_A18_mu ,
         RR_biomass = (biomass_mu-biomass_A18_mu) / biomass_A18_mu ,
         RR_shannon = (shannon-shannon_A18_mu) / shannon_A18_mu)

# Log absolute value Realized Disturbance (LRR response ratio)
df <- df %>%
  mutate(LRR_length = log(abs((length_mu-length_A18_mu) / length_A18_mu)) ,
         LRR_d100 = log(abs((d100_mu-d100_A18_mu) / d100_A18_mu)) ,
         LRR_biomass = log(abs((biomass_mu-biomass_A18_mu) / biomass_A18_mu)) ,
         LRR_shannon = log(abs((shannon-shannon_A18_mu) / shannon_A18_mu)))
...
df <- df %>% arrange(site_code, collection_date) %>% # density
  mutate(y_margin = qt(0.95, df=4-1)*d100_A18_sd / sqrt(4),
         LCL_d100 = d100_A18_mu - y_margin,
         UCL_d100 = d100_A18_mu + y_margin,
...
df <- df %>% arrange(site_code, collection_date) %>% # length
  mutate(y_margin = qt(0.95, df=4-1)*length_A18_sd / sqrt(4),
         LCL_length = length_A18_mu - y_margin,
         UCL_length = length_A18_mu + y_margin,
...
df <- df %>% arrange(site_code, collection_date) %>% # biomass
  mutate(y_margin = qt(0.95, df=4-1)*biomass_A18_sd / sqrt(4),
         LCL_biomass = biomass_A18_mu - y_margin,
         UCL_biomass = biomass_A18_mu + y_margin,
...
df <- df %>% arrange(site_code, collection_date) %>% # shannon
  mutate(y_margin = qt(0.95, df=4-1)*shannon_A18_sd / sqrt(4),
         LCL_shannon = shannon_A18_mu - y_margin,
         UCL_shannon = shannon_A18_mu + y_margin,
...
length_v_base18 <- df %>%
  ...
  geom_ribbon(aes(x=collection_date, ymin=LCL_length, ymax=UCL_length),
              fill = 'grey50', alpha=.2) +
  geom_smooth(aes(y=length_A18_mu), method = 'lm', 
...
d100_v_base18 <- df %>%
  ...
  geom_ribbon(aes(x=collection_date, ymin=LCL_d100, ymax=UCL_d100),
              fill = 'grey50', alpha=.2) +
  geom_smooth(aes(y=d100_A18_mu), method = 'lm', 
              color = 'black', linetype = 2, alpha = .5, show.legend = F) +
  geom_point(shape = 21, size = 4) +
...
biomass_v_base18 <- df %>%
  ...
  geom_ribbon(aes(x=collection_date, ymin=LCL_biomass, ymax=UCL_biomass),
              fill = 'grey50', alpha=.2) +
  geom_smooth(aes(y=biomass_A18_mu), method = 'lm', 
              color = 'black', linetype = 2, alpha = .5, show.legend = F) +
  geom_point(shape = 21, size = 4) +
  ylab('biomass (g/m2)') +
  xlab(element_blank())
...
shannon_v_base18 <- df %>%
  ...
  geom_ribbon(aes(x=collection_date, ymin=LCL_shannon, ymax=UCL_shannon),
              fill = 'grey50', alpha=.2) +
  geom_smooth(aes(y=shannon_A18_mu), method = 'lm', 
              color = 'black', linetype = 2, alpha = .5, show.legend = F) +
  geom_point(shape = 21, size = 4) +
...
RR_plot_18base <- df %>% 
  ...
  ggtitle('Response Ratio: baseline = 2018 annual average')
...
LRR_plot_18base <- df %>% 
  ...
  ggplot(aes(x=annualrain, y=y_val)) +
           ...
           stat_cor(label.y = 6, size = 5)+ 
           stat_regline_equation(label.y = 7, size = 5) +
           ylab('Max Log Response Ratio') +
           xlab('Annual Rainfall (cm)') +
  ggtitle('Max AbsoluteV Log Response Ratio: baseline = 2018 annual average')
...
RI_plot_18base <- df %>% 
  ...
  geom_smooth(method = "lm", se=FALSE, color="red", linetype=1,
              formula =  y ~ x) +
  geom_point(shape = 21, size = 4, alpha = .5, fill='grey50') +
  stat_cor(label.y = 130, size = 5)+ 
  stat_regline_equation(label.y = 150, size = 5) +
  ylim(0,175) +
  ylab('Days to Recovery') +
  xlab('Annual Rainfall (cm)') +
  ggtitle('Recovery Interval: days to 90% CL of baseline (2018 annual average)')

#------------------------------------------------------------------------------
# export
#------------------------------------------------------------------------------

write_csv(df, 'Data/response_v_2018_baseline.csv')

ggsave('Figures/Response_Ratios_2018base.pdf',
       plot = RR_plot_18base,
       width = 9,
       height = 9,
       units = c("in"))

ggsave('Figures/LRR_2018base.pdf',
       plot = LRR_plot_18base,
       width = 9,
       height = 9,
       units = c("in"))

ggsave('Figures/Recovery_Intervals_2018base.pdf',
       plot = RI_plot_18base,
       width = 9,
       height = 9,
       units = c("in"))

# End response_v_2018_baseline_calc