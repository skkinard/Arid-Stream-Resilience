...
# state data
state_map_data <- ne_states(country = 'united states of america', returnclass = "sf")

# rivers
rivers_tx <- read_sf(dsn = "data/clean_data/Map_Data/Surface_Water/Surface_Water.shp", layer = "Surface_Water")

# precip
precip <- read_sf(dsn = "Data/Map_Data/precip1981_2010_a_tx.shp", 
                  layer = "precip1981_2010_a_tx") %>%
  rename('Rainfall (cm/yr)' = 'Precip_cm')

# oceans
oceans <- read_sf(dsn = 'data/clean_data/Map_Data/natural_earth_vector/10m_physical/ne_10m_ocean.shp', layer = "ne_10m_ocean")

# cities
cities <- read_sf(dsn = 'data/clean_data/Map_Data/natural_earth_vector/10m_cultural/ne_10m_populated_places_simple.shp', 
...
# urban
urban <- read_sf(dsn = 'data/clean_data/Map_Data/natural_earth_vector/10m_cultural/ne_10m_urban_areas.shp', 
                 layer = "ne_10m_urban_areas")

# Site labels
d_site <- read_csv('data/clean_data/RAPID_lte.csv') %>%
...
site_map <- ggplot() +
  ...
  #geom_label_repel(data = d_site, aes(x = long, y = lat, label = Site),
